...
export default class Tokenizer {
  ...
  setPrevTokenActive() {
    if (!this.#isFirstTokenInHistory()) {
      const currentActiveTokenIndex =
        this.#getIndexOfCurrentActiveTokenInHistory();
  ...
  setNextTokenActive() {
    if (this.#currentTokenInHistory()) {
      this.#activeToken =
        this.#tokensHistory[this.#getIndexOfCurrentActiveTokenInHistory() + 1];
    } else {
  ...
  #getLongestMatchAndSliceString(tokens) {
    return tokens.reduce((prev, current) =>
      prev.value.length > current.value.length ? prev : current
    );
  }
